...
const eliminarUsuario = (req, res) => {
  ...
  UsuarioService.eliminar({ correo })
    .then(usuarioEliminado => {
      if (!usuarioEliminado) return res.status(200).json(errorResponse("NO_ENCONTRADO", `Usuario con correo ${correo} no encontrado`, null, 3));
      res.json({ mensaje: `Usuario ${usuarioEliminado.nombre} eliminado con éxito`, codigo: 0 });
    })
    .catch(error => res.status(200).json(errorResponse("ERROR_ELIMINAR", "Error al eliminar usuario", error.message, 3)));
};
...
const actualizarUsuario = (req, res) => {
  ...
  if (!correo) return res.status(200).json(errorResponse("FALTA_CORREO", "El correo del usuario a actualizar es requerido", null, 2));
  ...
  hashPromise
    ...
    .then(usuarioActualizado => {
      if (!usuarioActualizado) return res.status(200).json(errorResponse("NO_ENCONTRADO", `Usuario con correo ${correo} no encontrado`, null, 3));
      res.json({ mensaje: "Usuario actualizado con éxito", usuario: usuarioActualizado, codigo: 0 });
    })
    .catch(error => res.status(200).json(errorResponse("ERROR_ACTUALIZAR", "Error al actualizar usuario", error.message, 3)));
};
